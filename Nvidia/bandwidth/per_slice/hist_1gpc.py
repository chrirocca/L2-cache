...
with open('result1GPC.txt', 'r') as infile:
    slice_num = 0
    sm_num = 0
    for line in infile:
        line = line.strip()
...
ax.set_xlim(left=180, right=215)

# Set the x-ticks to be every 1 from 70 to 100
plt.xticks(np.arange(180, 216, 2))
...
plt.savefig("1gpc1slicehist.png", dpi=600, bbox_inches='tight')
plt.show()